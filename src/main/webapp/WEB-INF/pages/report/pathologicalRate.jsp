...
<div id="myPrintArea">
	...
	<table width="316" border="1" align="center" bordercolor="#000000">
		<tr>
			...
			<td width="150">不符</td>
			<td width="150">疑诊</td>
			<td width="150">诊断符合率</td>
		</tr>
		<c:forEach items="${dataDtoList }" var="dataDtoList">
		<tr>
			<td>&nbsp;${dataDtoList.month }</td>
			<td>&nbsp;<fmt:formatNumber type="number" value="${dataDtoList.zongShu }" maxFractionDigits="2"/></td>
			<td>&nbsp;<fmt:formatNumber type="number" value="${dataDtoList.fuHe }" maxFractionDigits="2"/></td>
			<td>&nbsp;<fmt:formatNumber type="number" value="${dataDtoList.buFu }" maxFractionDigits="2"/></td>
			<td>&nbsp;<fmt:formatNumber type="number" value="${dataDtoList.yiZhen }" maxFractionDigits="2"/></td>
			<c:choose>
				<c:when test="${dataDtoList.zongShu == 0 }">
					<td>&nbsp;<fmt:formatNumber type="number" value="0" maxFractionDigits="2"/></td>
				</c:when>
				<c:otherwise>
					<td>&nbsp;<fmt:formatNumber type="number" value="${(dataDtoList.fuHe/dataDtoList.zongShu) * 100 }" maxFractionDigits="2"/>%</td>
				</c:otherwise>
			</c:choose>
		</tr>
		</c:forEach>
...
$(function(){
	...
	$("#search").click(function(){
		...
		if(dateStart == "" || dateEnd == ""){
			...
			var url = "${contextPath }/report/search/dateStart/" + $("#dateStart").val() + "/dateEnd/" + $("#dateEnd").val() + "/parm/14";
